"""Persistência local dos orçamentos customizados em JSON (modo offline)."""
...
def carregar_arquivo():
    ...
    if dados.get("orcamento_ativo_id"):
        ...
        ids = {o.get("id") for o in dados.get("orcamentos", [])}
        if ativo not in ids:
            dados["orcamento_ativo_id"] = (
                dados["orcamentos"][0]["id"] if dados.get("orcamentos") else None
            )
...
def orcamento_para_dict(orcamento, *, criado_em=None, atualizado_em=None):
    ...
    dados["id"] = getattr(orcamento, "id", novo_id())
...
def adicionar_orcamento_importado(orcamento):
    dados = carregar_arquivo()
    payload = orcamento_para_dict(orcamento)
    payload["id"] = getattr(orcamento, "id", novo_id())
